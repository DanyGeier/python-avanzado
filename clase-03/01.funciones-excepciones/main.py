...
# Tupla = datos que deberian permancer iguales -> () -> No mutables
...
# -> 2, 5, 8, 9, 20, 44 -> promedio -> [suma de todos los valores] / [cantidad de elementos]
...
"""  def crear_perfil(**perfil):
    print("\nPerfil creado:")
    for clave, valor in perfil.items(): # py despaquetar -> desestructurando en js
        print(f"{clave} : {valor}")
...
obtener_longitud = lambda texto: len(texto)
print(obtener_longitud("Python"))
...
# Funciones de ordenar superior ->> recibe otra función como argumento o devuelve una función
...
resultado = aplicar(lambda n: n * 2, 20) # 40
print(resultado)
...
pares = filter(lambda numero: numero % 2 == 0, numeros) # devuelve una lista de solo los elementos que coincida con lo que se haya decidido en el callback
...
# mostrar_producto()
# descuento_aplicado = calcular_precio_fina(precio, descuento)
